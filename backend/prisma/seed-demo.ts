// Optional demo/sample dataset — NOT run at boot.
//
// `seed.ts` only guarantees the platform superadmin. This script re-adds the
// full demo tenant (staff users, clients, projects and one record per module)
// so an empty install has something to click through. It is:
//   • additive & idempotent — every write is an upsert or count-guarded, so it
//     never deletes or overwrites current data and is safe to run repeatedly;
//   • non-destructive to the superadmin — it touches the demo staff accounts
//     only, never admin@inspecta.africa.
//
// Run it explicitly:
//   npx tsx prisma/seed-demo.ts            (local)
//   docker exec inspecta-prod-backend npx tsx prisma/seed-demo.ts   (server)
import { Role, ProjectStatus, ProjectHealth } from '@prisma/client';
import { prisma } from '../src/lib/prisma';
import { hashPassword } from '../src/lib/password';
import { ensureDefaultRoles } from '../src/modules/roles/roles.service';
import { HOST_ORG_SLUG } from '../src/modules/auth/superadmin';

async function main() {
  console.log('🌱 Seeding INSPECTA BUILDOS demo data...');

  // Reuse the superadmin's billing-exempt host org as the demo tenant. `update:
  // {}` means an existing org (the live one) is left exactly as it is.
  const org = await prisma.organization.upsert({
    where: { slug: HOST_ORG_SLUG },
    update: {},
    create: {
      name: 'Inspecta GC Corp', slug: HOST_ORG_SLUG,
  ...
  const superHash = await hashPassword('Super@12345');

  // The tenant's editable org chart. Roles are data, so the demo org gets the
  // standard set (including Foreman) and every seeded account is put on one.
  ...
  // The live platform superadmin (admin@inspecta.africa) is intentionally NOT
  // in this list — it is owned by ensureSuperAdmin and left untouched.
  const userSpecs: Array<{ email: string; fullName: string; role: Role; roleKey?: string; hash: string; slot?: string }> = [
    { email: 'superadmin@inspecta.ai', fullName: 'Platform Superadmin', role: Role.PLATFORM_ADMIN, hash: superHash },
    { email: 'admin@inspecta.ai', fullName: 'Alex Thompson', role: Role.SYSTEM_ADMIN, roleKey: 'system-admin', hash: adminHash },
  ...
  } else if (alreadySeeded > 0) {
    console.log('   Module demo data already present — left as-is.');
  }

  console.log('✅ Demo seed complete.');
  console.log('   Demo users (password Demo@12345): pm@ / engineer@ / qs@ / store@ / foreman@ inspecta.ai');
  console.log('   admin@inspecta.ai / Admin@12345 · superadmin@inspecta.ai / Super@12345');
}
