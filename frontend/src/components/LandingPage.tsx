...
import MarketingLayout, { CORAL, INK, MAROON, DARK, WHATSAPP } from './marketing/MarketingLayout';
    ...
    <MarketingLayout>
      ...
      <section className="relative overflow-hidden px-5 md:px-10 pt-16 pb-20 md:pt-24 md:pb-28">
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          ...
          <p className="mt-6 text-base md:text-lg text-[var(--mk-muted)] max-w-2xl mx-auto leading-relaxed">
          ...
          <div className="mt-12 max-w-4xl mx-auto">
            <img src={MEDIA.heroImage} alt="Inspecta field crew surveying a completed road and bridge works in Rwanda" loading="lazy" className="w-full h-64 md:h-96 object-cover rounded-3xl shadow-xl" />
          </div>
        </div>
      </section>
      ...
      <section className="px-5 md:px-10 pb-4">
        <p className="max-w-3xl mx-auto text-center text-[var(--mk-muted)] text-sm md:text-base leading-relaxed border-y border-[var(--mk-border)] py-6">
      ...
      <section id="services" className="px-5 md:px-10 py-16 md:py-20 max-w-6xl mx-auto scroll-mt-20">
        <div className="text-center mb-12">
          ...
          <p className="mt-3 text-[var(--mk-muted)] text-sm">Three integrated disciplines, one accountable partner.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-6">
          {services.map((s) => {
            const Icon = s.icon;
            return (
              <Link key={s.title} to={s.to} className="group rounded-2xl border border-[var(--mk-border)] p-7 hover:shadow-xl transition-all hover:-translate-y-1 bg-[var(--mk-surface)] block">
                ...
                <p className="text-sm text-[var(--mk-muted)] leading-relaxed">{s.body}</p>
      ...
      {/* Why Inspecta */}
      <section id="why" className="px-5 md:px-10 py-16 md:py-20 scroll-mt-20" style={{ background: 'var(--mk-tint)' }}>
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            {whyPoints.map(([title, body]) => (
              <div key={title} className="flex items-start gap-4 bg-[var(--mk-surface)] rounded-xl p-5 border border-[var(--mk-border)]">
                <CheckCircle2 className="w-6 h-6 shrink-0 mt-0.5" style={{ color: CORAL }} />
                <div><h4 className="font-bold text-[var(--mk-fg)]">{title}</h4><p className="text-sm text-[var(--mk-muted)] mt-0.5">{body}</p></div>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="erp" className="px-5 md:px-10 py-16 md:py-20 scroll-mt-20">
        <div className="max-w-6xl mx-auto rounded-3xl overflow-hidden relative p-8 md:p-14 text-white" style={{ background: DARK }}>
      ...
      {/* Contact CTA */}
      <section className="px-5 md:px-10 py-16 md:py-24 text-center" style={{ background: 'var(--mk-tint)' }}>
