...
router.use(
  ...
  createCrudRouter({
    ...
    filterFields: ['category', 'budgetType'],
    sumFields: ['amount'],
    requireProject: true,
...
const cashCreate = z.object({
  ...
  amount: z.number().nonnegative(),
  isForecast: z.preprocess((v) => (v === 'true' ? true : v === 'false' ? false : v), z.boolean()).optional(),
...
router.use(
  ...
  createCrudRouter({
    ...
    dateField: 'date',
    filterFields: ['direction', 'category'],
    sumFields: ['amount'],
    orderBy: { date: 'desc' },
    transform: stamp,
  }),
